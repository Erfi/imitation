...
class BCEvalCallback:
    def __init__(
        self,
        eval_env: gym.Env,
        model: policies.ActorCriticPolicy,
        logger: SB3Logger = None,
        eval_freq: int = 100,
    ...
    def __call__(self, *args: Any, **kwds: Any) -> None:
        if self._num_steps % self.eval_freq == 0:
            ...
            if self.log_dir:
                # write results as npz file
                self.evaluations_timesteps.append(self._num_steps)
            ...
            if self.verbose >= 1:
                print(
                    f"Eval num_timesteps={self._num_steps}, "
            ...
            if self.logger:
                ...
                self.logger.record(
                    "time/total_timesteps", self._num_steps, exclude="tensorboard"
                )
                self.logger.dump(self._num_steps)
            ...
            if mean_reward > self.best_mean_reward:
                ...
                if self.best_model_save_path is not None:
                    th.save(
                        self.model,
                        os.path.join(self.best_model_save_path, "best_model.pt"),
                    )
